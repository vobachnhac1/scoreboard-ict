...
const LanguageSwitcher = ({ className = '', compact = false }) => {
  ...
  return (
    <div className={`relative inline-flex items-center justify-center ${className}`}>
      ...
      <div className={`flex ${compact ? 'flex-col' : 'flex-row items-center'} gap-1 bg-slate-100 dark:bg-slate-700/50 rounded p-1 shadow-inner border border-slate-200/50 dark:border-slate-600/50`}>
        {languages.map((lang) => (
          <button
            key={lang.code}
            onClick={() => handleChangeLanguage(lang.code)}
            className={`
              group relative inline-flex items-center justify-center ${compact ? 'px-2 py-2 w-10 h-10' : 'px-3 py-2 w-16 h-10'} rounded
