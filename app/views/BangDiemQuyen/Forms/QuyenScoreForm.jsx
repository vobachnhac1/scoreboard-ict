...
export default function QuyenScoreForm({
  ...
  useEffect(() => {
    ...
    if (currentScores.length > 0) {
      if (soGiamDinh === 5) {
        // Có 5 giám định: bỏ cao nhất, bỏ thấp nhất
        const minScore = Math.min(...currentScores);
        const maxScore = Math.max(...currentScores);
        // Trừ đi 1 min và 1 max
        const sumAll = currentScores.reduce((acc, curr) => acc + curr, 0);
        calculatedTotal = sumAll - minScore - maxScore;
      } else {
        // 3 giám định hoặc khác: cộng tất cả
  ...
  // Render Judge Row (plain function, not component - avoids re-mount stutter)
  const renderJudgeInput = (judgeNumber) => (
    <div key={`judge-input-${judgeNumber}`} className="space-y-1.5 w-full">
      <label
    ...
    <Fragment>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="bg-gray-50 dark:bg-gray-800 rounded p-5 border border-gray-200 dark:border-gray-700 shadow-sm relative overflow-visible">
          {/* <h3 className="font-bold text-sm text-gray-800 dark:text-gray-200 mb-5 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700 pb-3 flex items-center gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-gray-500" viewBox="0 0 20 20" fill="currentColor">
              <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
            </svg>
            CHI TIẾT ĐIỂM GIÁM ĐỊNH
          </h3> */}
