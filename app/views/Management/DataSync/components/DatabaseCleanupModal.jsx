...
const DatabaseCleanupModal = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-[3rem] max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden border border-rose-50 dark:border-rose-900/30">
        ...
        <div className="flex-1 overflow-auto p-8 custom-scrollbar bg-gray-50/30 dark:bg-gray-900/40">
          {/* ===== WARNING PANEL ===== */}
          <div className="mb-8 p-6 bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-800/50 rounded-[2rem] flex items-start gap-5">
            <div className="w-10 h-10 bg-rose-100 dark:bg-rose-900/50 rounded-xl flex items-center justify-center text-rose-600 dark:text-rose-400 shrink-0">
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                {[
                  ...
                  <div key={i} className={`p-4 rounded-2xl border-2 bg-white dark:bg-gray-800 ${stat.color === 'rose' && selectedTablesToDelete.length > 0 ? 'border-rose-100 dark:border-rose-900' : 'border-blue-50 dark:border-blue-900/30'}`}>
              ...
              {/* ===== TABLES LIST ===== */}
              <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] border border-blue-50 dark:border-blue-900/30 overflow-hidden">
                      ...
                      >
                        <td className="px-6 py-4">
                          {table.isSyncable ? (
                            ...
                            <div className={`w-8 h-8 rounded-lg border-2 flex items-center justify-center transition-all ${selectedTablesToDelete.includes(table.name) ? 'bg-rose-600 border-rose-600 text-white' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-transparent group-hover:border-rose-300'}`}>
                        ...
                        <td className="px-6 py-4 text-right">
                          {table.isSyncable ? (
                            <span className="px-3 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-lg text-[9px] font-black uppercase tracking-wider border border-emerald-100 dark:border-emerald-800/50">
                              {t("data_sync.fixed_system")}
                            </span>
                          ) : (
                            <span className="px-3 py-1 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-lg text-[9px] font-black uppercase tracking-wider border border-amber-100 dark:border-amber-800/50">
        ...
        <div className="px-10 py-6 border-t border-blue-50 dark:border-blue-900/30 bg-gray-50 dark:bg-gray-800/50 flex justify-end gap-3">
          <button
            onClick={onClose}
            className="px-8 py-4 bg-white dark:bg-gray-800 border-2 border-blue-50 dark:border-blue-900/30 hover:bg-blue-50 dark:hover:bg-blue-900/50 text-blue-600 dark:text-blue-400 rounded-2xl text-[11px] font-black uppercase tracking-widest active:scale-95 transition-all"
          >
          ...
          <button
            ...
            className="px-10 py-4 bg-rose-600 hover:bg-rose-700 disabled:bg-gray-300 dark:disabled:bg-gray-700 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest shadow-xl shadow-rose-500/20 active:scale-95 transition-all flex items-center gap-3"
          >
            {loadingCleanup ? (
