...
const CompDKDataTable = ({ value, fileName }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-4 p-4 bg-white dark:bg-gray-800 rounded-[2rem] border border-blue-50 dark:border-blue-900/30 ">
        <div className="flex items-center gap-3 px-4 py-2 bg-blue-50 dark:bg-blue-900/40 rounded-xl border border-blue-100 dark:border-blue-800">
          ...
          {q && (
            <span className="flex items-center gap-1.5 px-2 py-0.5 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 rounded-full text-[9px] font-black uppercase">
              <span className="w-1 h-1 bg-amber-500 rounded-full animate-pulse"></span> {t("data_sync.filtering")}
            </span>
          )}
        </div>
        ...
        <div className="flex-1 relative group">
          ...
          <input
            ...
            className="w-full pl-11 pr-10 py-3 text-xs font-bold bg-gray-50 dark:bg-gray-900 border-2 border-transparent focus:border-blue-500 focus:bg-white dark:focus:bg-gray-800 rounded-2xl transition-all outline-none text-blue-950 dark:text-blue-100 placeholder-blue-300/50"
          />
          {search && (
            <button
              onClick={() => setSearch("")}
        ...
        <button
          onClick={exportCSV}
          className="px-6 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest  active:scale-95 transition-all flex items-center gap-3"
      ...
      {/* ===== TABLE CONTAINER ===== */}
      <div className="rounded-[2.5rem] border-2 border-blue-50 dark:border-blue-900/30 bg-white dark:bg-gray-800  overflow-hidden">
                  ...
                  <tr>
                    <td colSpan={headers.length + 1} className="px-6 py-20 text-center">
                      <div className="flex flex-col items-center gap-4">
                        <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900/30 rounded-3xl flex items-center justify-center text-blue-200 dark:text-blue-800">
...
const DATA_FIELDS = ["data", "config_system", "scores", "referrers"]; // Các field hiển thị ở tab riêng
...
const RecordDetailModal = ({ show, record, onClose }) => {
  ...
  const configValue = record.config_system;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-[3rem] w-full max-w-7xl max-h-[90vh] flex flex-col overflow-hidden border border-blue-50 dark:border-blue-900/30">
          ...
          <div className="w-72 flex-shrink-0 border-r border-blue-50 dark:border-blue-900/30 bg-white/50 dark:bg-gray-800/20 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2 custom-scrollbar">
              {metaEntries.map(([key, value]) => (
                <div key={key} className="p-4 rounded-2xl hover:bg-white dark:hover:bg-gray-800 transition-all border border-transparent hover:border-blue-50 dark:hover:border-blue-900/30 group">
            ...
            {(hasData || hasConfig || hasScores || hasReferrers) ? (
              <>
                ...
                <div className="px-8 py-4 bg-white dark:bg-gray-950/20 border-b border-blue-50 dark:border-blue-900/30 flex items-center justify-between shrink-0">
                  <div className="flex items-center gap-2 p-1 bg-blue-50 dark:bg-gray-900 rounded-2xl border border-blue-100 dark:border-blue-800 overflow-x-auto">
                    {hasData && (
                      <button
                        onClick={() => setActiveTab("table")}
                        className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "table"
                    ...
                    {hasConfig && (
                      <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "config"
                          ? "bg-white dark:bg-gray-800 text-purple-600 dark:text-purple-400  scale-105"
                          : "text-purple-400/60 hover:text-purple-600"
                          }`}
                      >
                        ⚙️ {t("data_sync.config")}
                      </button>
                    )}
                    {hasScores && (
                      <button
                        onClick={() => setActiveTab("scores")}
                        className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "scores"
                          ? "bg-white dark:bg-gray-800 text-emerald-600 dark:text-emerald-400  scale-105"
                          : "text-emerald-400/60 hover:text-emerald-600"
                          }`}
                      >
                        🏆 {t("data_sync.scores")}
                      </button>
                    ...
                    {hasReferrers && (
                      <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "referrers"
                          ? "bg-white dark:bg-gray-800 text-amber-600 dark:text-amber-400  scale-105"
                          : "text-amber-400/60 hover:text-amber-600"
                          }`}
                      >
                    ...
                    <button
                      ...
                      className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === "json"
                ...
                <div className="flex-1 overflow-auto p-8 custom-scrollbar">
                  {activeTab === "table" && hasData ? (
                    ...
                    <div className="animate-in fade-in zoom-in-95 duration-500">
                      <div className="bg-purple-50 dark:bg-purple-900/20 rounded-[2.5rem] border-2 border-purple-100 dark:border-purple-800 p-8">
                        <div className="flex items-center gap-3 mb-6">
                          ...
                          <h3 className="text-xl font-black text-purple-700 dark:text-purple-300 uppercase">{t("data_sync.config")}</h3>
                        </div>
                        <pre className="font-mono text-xs p-6 bg-white dark:bg-gray-900 text-purple-900 dark:text-purple-200 rounded-2xl border border-purple-200 dark:border-purple-700 overflow-auto custom-scrollbar leading-relaxed max-h-[600px]">
                    ...
                    <div className="animate-in fade-in zoom-in-95 duration-500">
                      <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-[2.5rem] border-2 border-emerald-100 dark:border-emerald-800 p-8">
                        <div className="flex items-center gap-3 mb-6">
                          ...
                          <h3 className="text-xl font-black text-emerald-700 dark:text-emerald-300 uppercase">{t("data_sync.scores")}</h3>
                        </div>
                        <pre className="font-mono text-xs p-6 bg-white dark:bg-gray-900 text-emerald-900 dark:text-emerald-200 rounded-2xl border border-emerald-200 dark:border-emerald-700 overflow-auto custom-scrollbar leading-relaxed max-h-[600px]">
                          {JSON.stringify(scoresValue, null, 2)}
                        </pre>
                      </div>
                    </div>
                  ) : activeTab === "referrers" && hasReferrers ? (
                    <div className="animate-in fade-in zoom-in-95 duration-500">
                      <div className="bg-amber-50 dark:bg-amber-900/20 rounded-[2.5rem] border-2 border-amber-100 dark:border-amber-800 p-8">
                        <div className="flex items-center gap-3 mb-6">
                          <div className="w-10 h-10 bg-amber-500 rounded-xl flex items-center justify-center text-white">
                            <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" /></svg>
                          </div>
                          <h3 className="text-xl font-black text-amber-700 dark:text-amber-300 uppercase">{t("data_sync.referees")}</h3>
                        </div>
                        <pre className="font-mono text-xs p-6 bg-white dark:bg-gray-900 text-amber-900 dark:text-amber-200 rounded-2xl border border-amber-200 dark:border-amber-700 overflow-auto custom-scrollbar leading-relaxed max-h-[600px]">
                          {JSON.stringify(referrersValue, null, 2)}
                        </pre>
                      </div>
                    </div>
                  ) : activeTab === "json" ? (
                    <div className="animate-in fade-in zoom-in-95 duration-500 h-full">
                      <pre className="h-full font-mono text-xs p-8 bg-blue-950 dark:bg-black text-blue-200 dark:text-emerald-400 rounded-[2.5rem] border border-blue-900/50 overflow-auto custom-scrollbar leading-relaxed">
              ...
              <div className="flex-1 flex flex-col p-8 overflow-hidden">
                ...
                <div className="flex-1 overflow-hidden rounded-[2.5rem] border border-blue-50 dark:border-blue-900/30 ">
        ...
        <div className="px-10 py-6 border-t border-blue-50 dark:border-blue-900/30 bg-white dark:bg-gray-800 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">
            ...
            {(hasData || hasConfig || hasScores || hasReferrers) && (
              <div className="flex items-center gap-2">
                ...
                {hasConfig && (
                  <div className="flex items-center gap-1.5 px-3 py-1 bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-full border border-purple-100 dark:border-purple-800">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                    {t("data_sync.config")}
                  </div>
                )}
                {hasScores && (
                  <div className="flex items-center gap-1.5 px-3 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-full border border-emerald-100 dark:border-emerald-800">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                    {t("data_sync.scores")}
                  </div>
                )}
                {hasReferrers && (
                  <div className="flex items-center gap-1.5 px-3 py-1 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-full border border-amber-100 dark:border-amber-800">
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
                    {t("data_sync.referees")}
                  </div>
                )}
          ...
          <div className="flex gap-3">
            <button
              onClick={onClose}
              className="px-8 py-3 bg-white dark:bg-gray-800 text-blue-600 dark:text-blue-400 rounded-2xl text-[10px] font-black uppercase tracking-widest border-2 border-blue-50 dark:border-blue-900/30 hover:bg-blue-600 hover:text-white transition-all active:scale-95"
            >
