...
const StagingSection = ({
  ...
  const TAB_NAME = {
    ...
    if (!matchedLocal) return false;
    const incomingData = staging.record_data;
    const p1 = String(incomingData.sheet_name || "").substring(0, 3).toUpperCase();
    const p2 = String(matchedLocal.sheet_name || "").substring(0, 3).toUpperCase();
    return p1 && p2 && p1 !== p2;
  });
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 bg-white dark:bg-gray-800 rounded border border-blue-50 dark:border-blue-900/30 ">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-blue-100 dark:bg-blue-900 rounded flex items-center justify-center text-blue-600 dark:text-blue-400 ">
        ...
        <div className="flex items-center gap-3">
          {stagingView === "review" && (
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all active:scale-95 border border-blue-100 dark:border-blue-800"
            >
          ...
          <button
            ...
            className="group flex items-center gap-2 px-6 py-3 bg-white dark:bg-gray-800 text-blue-600 dark:text-blue-400 rounded text-[10px] font-black uppercase tracking-widest hover:border-blue-300 transition-all border-2 border-blue-50 dark:border-blue-900/30  active:scale-95"
          >
      ...
      {stagingView === "sessions" && (
        <div className="space-y-4">
          {stagingSessions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 bg-white/30 dark:bg-gray-800/20 rounded border-2 border-dashed border-blue-100 dark:border-blue-900/30">
            ...
            <div className="grid grid-cols-1 gap-4">
              {stagingSessions.map((session) => (
                <div key={session.session_id} className="group relative bg-white dark:bg-gray-800 border border-blue-50 dark:border-blue-900/30 rounded p-6 hover:border-blue-200 transition-all duration-300 overflow-hidden">
                  ...
                  <div className="relative z-10 flex flex-col lg:flex-row items-center gap-6">
                    <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900 rounded flex flex-col items-center justify-center border border-blue-100 dark:border-blue-800">
                    ...
                    <div className="flex gap-3 w-full lg:w-fit">
                      <button
                        ...
                        className="flex-1 lg:flex-none px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-[10px] font-black uppercase tracking-widest  active:scale-95 transition-all"
                      >
                        {t("data_sync.view_details")}
                      </button>
                      <button
                        onClick={() => handleDeleteSession(session.session_id)}
                        className="p-2 bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 hover:bg-rose-500 hover:text-white rounded transition-all active:scale-95 border border-rose-100 dark:border-rose-900/30"
      ...
      {stagingView === "review" && activeSession && (
        <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
          {loadingStaging ? (
            ...
            <>
              {/* Review Control Bar */}
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 p-6 bg-white dark:bg-gray-800 rounded border border-blue-50 dark:border-blue-900/30 ">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 bg-blue-50 dark:bg-blue-900 rounded flex items-center justify-center text-blue-600">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className={`px-4 py-2 rounded text-[10px] font-black uppercase tracking-widest transition-all ${!stagingTableFilter ? "bg-blue-600 text-white " : "bg-blue-50 dark:bg-blue-900/30 text-blue-400 hover:text-blue-600"}`}
                  >
                    {t("data_sync.all")}
                  </button>
                  {[...new Set(stagingData.map((r) => r.table_name))].map((t) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded text-[10px] font-black uppercase tracking-widest transition-all ${stagingTableFilter === t ? "bg-blue-600 text-white " : "bg-blue-50 dark:bg-blue-900/30 text-blue-400 hover:text-blue-600"}`}
                    >
                ...
                <button
                  ...
                  disabled={applyingStaging || hasCriticalMismatch}
                  className={`px-8 py-3 bg-emerald-600 hover:bg-emerald-700 disabled:bg-gray-400 text-white rounded text-[10px] font-black uppercase tracking-[0.2em] transition-all active:scale-95 flex items-center gap-3 ${hasCriticalMismatch ? "grayscale cursor-not-allowed opacity-50" : ""}`}
                >
                  {applyingStaging ? (
                    ...
                    <><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg> {t("data_sync.type_mismatch_error")}</>
                  ...
                  <table className="w-full text-sm border-collapse">
                    <thead>
                      <tr className="bg-blue-50/50 dark:bg-gray-900/50 border-b border-blue-100 dark:border-blue-900/30">
                        <th className="px-4 py-2 text-left text-[10px] font-black text-blue-400 uppercase tracking-widest w-40">{t("data_sync.module")}</th>
                        <th className="px-4 py-2 text-left text-[10px] font-black text-blue-600 uppercase tracking-widest w-[38%] border-x border-blue-100 dark:border-blue-900/30">{t("data_sync.source_machine_data")}</th>
                        <th className="px-4 py-2 text-left text-[10px] font-black text-emerald-600 uppercase tracking-widest w-[38%]">{t("data_sync.target_machine_data")}</th>
                        <th className="px-4 py-2 text-center text-[10px] font-black text-blue-400 uppercase tracking-widest">{t("data_sync.processing_flow")}</th>
                      </tr>
                    </thead>
                      ...
                      {stagingData
                        ...
                        .map((staging) => {
                          ...
                          const keys = Object.keys(incomingData).filter(
                            (k) => !HIDDEN_DETAIL_KEYS.includes(k) && k !== "id" && k !== "data" && k !== "items" && k !== "lkey"
                          );

                          return (
                            <tr key={staging.id} className={`transition-colors ${mapping.action === "skip" ? "opacity-40 grayscale" : "hover:bg-blue-50/30 dark:hover:bg-blue-900/5"}`}>
                              {/* Module Chip */}
                              <td className="px-6 py-6 align-top">
                                <div className="space-y-3">
                                  <span className="inline-block px-3 py-1 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 rounded text-[9px] font-black uppercase tracking-widest">{TAB_NAME[staging.table_name] || staging.table_name}</span>
                                  {staging.meta?.type === "partial_rows" && (
                                    <div className="flex flex-col gap-1.5 p-3 bg-blue-50/50 dark:bg-blue-900/20 rounded border border-blue-100/50 dark:border-blue-800/30">
                              ...
                              <td className="px-6 py-6 align-top border-x border-blue-100 dark:border-blue-900/30">
                                <div className="space-y-2">
                                  ...
                                  <button onClick={() => setStagingDetailRecord({ type: "incoming", data: incomingData })} className="mt-4 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 rounded text-[9px] font-black uppercase tracking-[0.1em] hover:bg-blue-600 hover:text-white transition-colors">
                              ...
                              <td className="px-6 py-6 align-top">
                                {mapping.action === "insert" ? (
                                  <div className="h-24 flex items-center justify-center p-4 bg-blue-50/30 dark:bg-blue-900/10 rounded border-2 border-dashed border-blue-100 dark:border-blue-800">
                                  ...
                                  <div className="space-y-2 animate-in fade-in duration-300">
                                    {keys.map((k) => {
                                      ...
                                      if (k === 'sheet_name') {
                                        const p1 = String(incomingData[k] || "").substring(0, 3).toUpperCase();
                                        const p2 = String(matchedLocal[k] || "").substring(0, 3).toUpperCase();
                                        if (p1 && p2 && p1 !== p2) isPrefixMismatch = true;
                                      }

                                      return (
                                        <div key={k} className={`flex gap-3 pb-2 border-b border-blue-50/50 dark:border-blue-900/10 last:border-0 last:pb-0 ${isPrefixMismatch ? "bg-rose-500/10 -mx-2 px-2 rounded" : isDiff ? "bg-amber-500/5 -mx-2 px-2 rounded" : ""}`}>
                                          <span className="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest w-24 shrink-0 pt-0.5">
                                            {t(`data_sync.meta_labels.${k}`, { defaultValue: k.charAt(0).toUpperCase() + k.slice(1).replace(/_/g, ' ') })}
                                          </span>
                                          <div className="flex-1 min-w-0">
                                            <div className={`text-xs font-bold break-words line-clamp-2 ${isPrefixMismatch ? "text-rose-600 dark:text-rose-400" : isDiff ? "text-amber-600 dark:text-amber-400 underline underline-offset-4 decoration-dotted" : "text-emerald-700 dark:text-emerald-400"}`}>
                                              {String(matchedLocal[k] ?? "—")}
                                            </div>
                                            {isPrefixMismatch ? (
                                              <span className="text-[8px] font-black text-rose-500 uppercase tracking-tighter">{t("data_sync.type_mismatch_error")}</span>
                                            ) : isDiff && (
                                              <span className="text-[8px] font-black text-amber-500 uppercase tracking-tighter">{t("data_sync.data_mismatch")}</span>
                                            )}
                                          </div>
                                        </div>
                                      );
                                  ...
                                  <div className="h-24 flex items-center justify-center p-4 bg-rose-50/30 dark:bg-rose-900/10 rounded border-2 border-dashed border-rose-100 dark:border-rose-900/30">
                              ...
                              <td className="px-6 py-6 align-top">
                                <div className="flex flex-col gap-3 items-center">
                                  <button
                                    onClick={() => setEditingMappingRecord(staging)}
                                    className={`px-4 py-2 rounded text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 flex items-center gap-2 ${mapping.action === "skip"
                                  ...
                                  >
                                    ...
                                    {mapping.action === "skip" ? t("data_sync.ignore") : mapping.action === "update" ? t("data_sync.update") : t("data_sync.insert")}
                                  </button>

                                  {mapping.action === "update" && mapping.mapping_to_id && (
                                    <span className="text-[9px] font-black text-amber-600 dark:text-amber-400 uppercase tracking-tighter">
                                      {t("data_sync.target")}: ID {mapping.mapping_to_id}
                                    </span>
                                  )}
                                </div>
                              </td>
